...
auto main() -> int {
  ...
  const auto async_fn0 = [](cotask::TaskScheduler &) -> cotask::Task<void> {
    for (auto i = 0; i < 10; ++i) {
  ...
  const auto async_fn1 = [](cotask::TaskScheduler &ts) -> cotask::Task<void> {
    ...
    std::cout << "async_fn1 co_return\n";
    co_return;
  };

  const auto async_fn2 = [=](cotask::TaskScheduler &ts) -> cotask::Task<void> {
